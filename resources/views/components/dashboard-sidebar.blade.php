<aside 
    ...
    <div class="flex items-center h-16 px-5 border-b border-slate-100 bg-white shrink-0 overflow-hidden">
        <a href="{{ route('home') }}" class="flex items-center gap-3 shrink-0">
            ...
            {{-- Brand text & tagline - hidden when collapsed --}}
            <div x-show="sidebarOpen" x-transition.opacity.duration.300ms class="leading-tight text-left">
                <span class="font-['Playfair_Display',serif] font-black text-lg text-page-text whitespace-nowrap block">
    ...
    <nav :class="sidebarOpen ? 'overflow-y-auto' : 'overflow-visible'" 
         ...
         x-data="{ activeItem: '{{ request()->routeIs('home') ? 'Home' : (request()->routeIs('discover') ? 'Discover' : (request()->routeIs('ai.planner') ? 'AI Planner' : (request()->routeIs('trips.*') ? 'My Trips' : (request()->routeIs('profile.*') ? 'Profile' : '')))) }}' }">
        @php
            $navItems = [
                ...
                ['name' => 'Discover', 'route' => 'discover', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9 9 0 100-18 9 9 0 000 18zm0-15v3m0 6v3m-6-6h3m6 0h3m-5.121-5.121l4.242 4.242" />'],
    ...
    <div class="p-3 border-t border-slate-100 overflow-hidden shrink-0">
        @if(Auth::user()->plan === 'plus')
            <div class="bg-pink-50/70 border border-party-1/10 rounded-2xl p-4 transition-all duration-300">
                <div class="flex items-start gap-3">
                    <div class="w-6 h-6 rounded-full bg-party-1 flex items-center justify-center shrink-0 text-white text-[10px]">
                        ★
                    </div>
                    <div x-show="sidebarOpen" x-transition.opacity.duration.300ms class="flex flex-col text-left">
                        <span class="text-xs font-black text-party-1 uppercase tracking-wider whitespace-nowrap">Plus Member</span>
                        <p class="text-[10px] text-slate-500 leading-tight mt-1">Unlock exclusive curated itineraries.</p>
                    </div>
                </div>
            </div>
        @else
        ...
        <div class="mt-4 px-1 space-y-1">
            <a href="{{ route('profile.show') }}" 
               class="flex items-center gap-4 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors group relative">
                <div class="shrink-0 w-6 h-6 flex items-center justify-center text-slate-400 group-hover:text-slate-600">
                    <svg class="w-5.5 h-5.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                    </svg>
                </div>
                <span x-show="sidebarOpen" x-transition.opacity.duration.300ms class="whitespace-nowrap overflow-hidden">Settings</span>
            ...
            <a href="mailto:support@triptogether.com" 
               class="flex items-center gap-4 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors group relative">
                <div class="shrink-0 w-6 h-6 flex items-center justify-center text-slate-400 group-hover:text-slate-600">
                    <svg class="w-5.5 h-5.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <span x-show="sidebarOpen" x-transition.opacity.duration.300ms class="whitespace-nowrap overflow-hidden">Help</span>
