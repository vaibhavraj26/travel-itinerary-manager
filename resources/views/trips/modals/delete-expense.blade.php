    ...
    <div x-show="showDeleteModal" 
        ...
        <div @click.away="showDeleteModal = false" 
            ...
            <div class="flex gap-3">
                <button @click="showDeleteModal = false" class="flex-1 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-colors">
                    Cancel
                </button>
                <form x-show="deletingExpense" :action="`/trips/{{ $trip->id }}/expenses/${deletingExpense}`" method="POST" class="flex-1">
                    @csrf
                    @method('DELETE')
